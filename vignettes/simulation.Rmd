...
The resources available for conservation are limited. To ensure that conservation resources are allocated cost-effectively, conservation plans (termed prioritizations) can be developed -- using a combination of economic, biodiversity, and land-use data -- that prioritize a set of sites for conservation management (e.g. protected area establishment). However, existing data on biodiversity patterns is incomplete. As a consequence, spatial prioritizations can potentially be improved by collecting additional data (i.e. surveying sites to refine estimates of which species are present inside them). However, this is further complicated by the fact that surveying sites consumes limited resources that could be used to manage sites for conservation. Thus decision makers need to strategically prioritize sites for surveys, based on the possibility that such surveys will substantially improve subsequent spatial prioritisations---this is not a trivial task.

The _surveyvoi_ package is decision support tool for prioritizing sites for surveys based on their potential to improve spatial prioritizations for managing biodiversity. Given a set of sites that could potentially be acquired for conservation management -- wherein some sites have previously been surveyed and other sites have not -- this package aims to identify which sites should be surveyed because doing so could lead to vastly superior conservation management plans. Methods are provided to calculate the _expected value of the decision (i.e. spatial prioritization) given current information_ (`evdci` function), the _expected value of the decision given perfect information_ (`evdpi` function), and the _expected value of the decision given a survey scheme_ (`evdsi` function). Furthermore, by exploring various survey schemes and computing the expected value of the decision associated with each scheme, optimal survey schemes can be identified (`optimal_survey_scheme` function). This package requires the [_Gurobi_ software suite](https://www.gurobi.com). In this vignette, we will use a simulated dataset to explore these methods.

## Simulate data

Let's simulate some data. To keep things simple, we will simulate data for 30 sites and one conservation feature (e.g. species). Of the 30 sites in total, we will simulate survey data for 15 sites---meaning that half of the sites will not have survey data. We will also simulate two spatially auto-correlated variables to characterise the environmental conditions within the sites. Although the simulation code (i.e. `simulate_site_data`) can output the probability that features are expected to inhabit the sites, we will disable this option to make our simulation study more realistic and instead predict these probabilities using statistical models.

```{r}
# simulate site data
site_data <- simulate_site_data(
  n_sites = 30, n_features = 1, proportion_of_sites_missing_data = 0.5,
...
The `feature_data` object is a table (i.e. `tibble`) that contains information on the conservation feature. Here, each row corresponds to a different feature -- and so it only has one row because we only have one feature -- and each column contains different information about the feature(s). The `name` column contains the name of the feature. The `survey` column indicates if the feature will be examined in future surveys. The `alpha` and `gamma` columns contain values that specify the conservation benefit gained when sites are managed for conservation (e.g. sites are prioritised for protected area establishment; we will explore this further below). Finally, the `sensitivity` and `specificity` columns denote the sensitivity (probability of correctly detecting a presence) and specificity (probability of correctly detecting an absence) of the survey methodology.

## Modelling probability of occupancy

After simulating the data, we need to estimate the probability of the feature occurring in the unsurveyed sites. This is important for calculating the return on investment of surveying sites, because if we can reliably predict the probability of the feature(s) occurring in unsurveyed sites using models then we don't really need to survey them. Specifically, we will fit gradient boosted regression trees -- via the [xgboost R package](https://xgboost.readthedocs.io/en/latest/) -- using functions contained in this package. These models are well-suited for modelling species distributions because they can accommodate high order interactions among different predictor variables that are needed to effectively model species' environmental niches. Since gradient boosted regression trees have many parameters that influence model performance, we will first run a calibration procedure to identify suitable parameters.

Our calibration procedure will use a random search method to identify suitable parameters. Given candidate values for different parameters, this method explores a predetermined number of randomly selected combinations of parameters (set using the `n_random_search_iterations` parameter). Ideally, we would examine all possible combinations of tuning parameters but this is not computationally feasible and the random search method is often "good enough". For a given combination of tuning parameters, the parameters are used to fit a model along with _k_-fold cross-validation, and the area under the curve (AUC) statistic derived from the held out data is used to assess model performance. After evaluating the predetermined number of parameter combinations, this function returns the best parameter combination (i.e. the parameters producing a model with the greatest AUC statistic).

```{r}
# create list of candidate parameter value for calibration procedure
candidate_xgb_params <- list(
  max_depth = seq(1, 10, 1), eta = seq(0.1, 0.5, 0.1),
  nrounds = seq(1, 100, 10), lambda = 10 ^ seq(-1.0, 0.0, 0.25),
  subsample = seq(0.5, 1.0, 0.1), colsample_bytree = seq(0.4, 1.0, 0.1),
  objective = "binary:logistic")

# preview candidate parameters
str(candidate_xgb_params)
...
xgb_params <- tune_occupancy_models(site_data, "f1", c("e1", "e2"),
                                   n_folds = 2,
                                   n_random_search_iterations = 1000,
                                   early_stopping_rounds = 10,
                                   parameters = candidate_xgb_params,
                                   n_threads = 1)
...
Now that we have identified suitable parameters for fitting the occupancy model, we will use these parameters to predict the probability that the feature occurs in the sites.
...
# since this value is greater than 0.7, our model is "good enough"
stopifnot(xgb_model$performance$test_auc_mean >= 0.7)
print(xgb_model$performance$test_auc_mean)
...
feature_data$model_sensitivity <- xgb_model$performance$test_sensitivity_mean
feature_data$model_specificity <- xgb_model$performance$test_specificity_mean
...
After simulating and modelling the data, we will now examine the _expected value of the decision given current information_. This value represents the conservation benefit of the optimal spatial prioritization given current information, whilst accounting for uncertainty in the presence/absence of the conservation feature in each site. Here, current information corresponds to our existing survey data and also our modelled probabilities of occupancy for the conservation feature.

The ($B$) conservation benefit accrued by managing sites for conservation is calculated using the follow equation. Note that this benefit corresponds to sites prioritised for conservation management, and not sites prioritised for surveys. In this equation, let $J$ denote the set of sites (indexed by $j$), and $I$ denote the set of feature (indexed by $i$). Also let $X_j$ contain binary (0/1) values indicating if each site $i \in I$ is prioritized for conservation management or not, $P_{ij}$ denote the probability of features $i \in I$ occurring in sites $j \in J$ (which we will estimate below), $\alpha_i$ denote the `alpha` column value(s), and $\gamma_i$ denote the `gamma` column value(s). This equation represents the objective function of the conservation prioritization component of the value of information calculations. It is conceptually similar to the objective function used in the Zonation decision support tool [@r1].

$$
B = \sum_{i \in I} \alpha_i \times \big( \sum_{j \in J} P_{ij} X_j \big) ^{\gamma_i}
$$

We can visualize this equation with a graph.

```{r, fig.height = h, fig.width = w}
# create table with plotting values
eqn_data <- tibble(x = seq(0, nrow(site_data), length.out = 1000),
                   y = feature_data$alpha * x ^ feature_data$gamma)

# create plot
ggplot(eqn_data, aes(x = x, y = y)) +
  geom_line(color = "#3366FF") +
  xlab("Sum(probabilities) held in conservation prioritization") +
  ylab("Conservation benefit") +
  labs()
```

We can see that conservation benefit increases with the expected amount of the conservation feature held in the prioritization (i.e. the line is always increasing). We can also see that the line has a bend in it, and so the rate of conservation benefit accrued decreases as more of the feature is conserved. This is useful if we have multiple features, because it tells the conservation prioritization algorithm that once we have conserved "enough" of one feature that it can then focus on other features (broadly speaking).
...
Given the total budget, we now have the data needed to calculate the _expected value of the decision given current information_. Unfortunately, we can only calculate this value exactly for very small problems (e.g. less than 10 planning units and one feature). Therefore, we will estimate this value using the _approximation_ method. This method provides multiple estimates that we can use to calculate the _approximate expected value of the decision given current information_.

```{r}
# approximate expected value of the decision given current information
# here, we will use 10 replicates, and for each replicate we will use
# 10'000 different combinations of presence/absences of the feature in the
...
approx_evd_current <- approx_evdci(
  ...
  n_approx_states_per_replicate = 10)

# preview values
str(approx_evd_current)

# print average value
print(mean(approx_evd_current))
```

We can potentially improve the _expected value of the decision given current information_ by learning more about which sites are more likely or less likely to contain the conservation.

## Expected value given perfect information

We can also explore what happen if we had perfect information. That is, how good would our management decision be if we had omniscient knowledge and knew exactly which planning units contained the feature and which planning units did not? This value is termed the _expected value of the decision given perfect information_. Although we would rarely have perfect information -- even after conducting additional surveys -- this metric is useful to understand the limit of how much the management decision could potentially improved given budgetary constraints. Indeed, the _expected value of the decision given current information_ can be very similar to _expected value of the decision given perfect information_ in some situations. Similar to before, this metric can only be calculated for very small problems, and so we will estimate it using the _approximation method_.
...
# preview values
str(approx_evd_perfect)

# print average value
print(mean(approx_evd_perfect))

# estimate how much perfect information would improve the decision
mean_improvement_given_perfect <- mean(approx_evd_perfect - approx_evd_current)
print(mean_improvement_given_perfect)
```
...
Although the _approximate expected value of the decision given perfect information_ tells us the likely performance of the management decision given omniscient knowledge, it doesn't actually tell us which sites we should survey to improve the expected value of the management decision.

## Survey schemes

Now we will generate some candidate survey schemes for potentially improving the management decision. To achieve this, we will set a budget for surveying additional sites. Specifically, this survey budget (i.e. `survey_budget`) will be set as 40% of the survey costs for the unsurveyed sites. Note that our total budget must always be greater than or equal to the survey budget.
...
Now let's visualize the different schemes.
...
Now that we've generated the survey schemes, let's calculate their return on investment using value of information analyses.
...
approx_evd_survey_schemes$value <- lapply(
...
# specifically, how much the information gained from each survey scheme is
# expected to improve management decision based on current information,
# whilst accounting for survey costs
approx_evd_survey_schemes$mean_improvement <-
  sapply(seq_len(nrow(approx_evd_survey_schemes)), function(i) {
    mean(approx_evd_survey_schemes$value[[i]] - approx_evd_current)
})
...
approx_evd_survey_schemes %>%
  mutate(name = factor(name, levels = name),
         is_best = mean_improvement == max(mean_improvement)) %>%
  ggplot(aes(x = name, y = mean_improvement)) +
    geom_col(aes(fill = is_best, color = is_best)) +
    geom_hline(yintercept = mean_improvement_given_perfect,
...
In this particular simulation, we can see that conducting additional surveys can improve our ability to decide which sites we should manage for conservation. This is evidenced by the fact that some of the survey schemes that survey at least one site (i.e. `cheap_scheme`, `geo_scheme`, `env_scheme`) have greater return on investment values than the scheme that does not survey any sites (`none_scheme`). We can also see that surveying additional sites may not necessarily improve our conservation decisions, since some of the survey schemes that survey at least one site (`rich_scheme`, `unc_scheme`) have smaller values than the scheme that does not survey any sites (`none_scheme`). Moreover, of the six scheme that we investigated, we can see that the scheme generated by surveying sites with distinct environmental conditions (`env_scheme`) is the best approach. But is it the best possible -- the optimal -- survey scheme?

## Optimal survey scheme

Now let's identify the optimal survey scheme by directly maximizing the expected value of the decision given a survey scheme. As discussed earlier, we this problem is too large to calculate the _expected value of the decision for a given survey scheme_ within a short period of time and so we use the approximation method to achieve this.
...
We can see that the optimal survey scheme (`opt_scheme`) is different to the scheme based on surveying sites with distinct environmental conditions (`env_scheme`). Interestingly, the optimal scheme selects fewer sites than the other schemes. This result shows that survey strategies need to ensure adequate funds remain for actually achieving conservation objectives.

```{r, fig.height = h, fig.width = w * 1.5}
# calculate return on investment of the optimal scheme
evd_opt <- approx_evdsi(
    site_data = site_data,
    feature_data = feature_data,
    site_occupancy_columns = "f1",
    site_probability_columns = "p1",
    ...
    feature_survey_sensitivity_column = "survey_sensitivity",
    feature_survey_specificity_column = "survey_specificity",
    model_survey_sensitivity_column = "model_sensitivity",
    model_survey_specificity_column = "model_specificity",
    feature_alpha_column = "alpha",
    feature_gamma_column = "gamma",
    total_budget = total_budget,
    xgb_parameters = xgb_params,
...
evd_survey_schemes <- rbind(
  evd_survey_schemes,
  tibble(name = "opt_scheme", value = list(evd_opt),
         mean_improvement = mean(evd_opt - approx_evd_current)))
...
We can see that the optimal survey scheme has the highest return on investment of all the candidate survey schemes. To understand how suboptimal the candidate survey schemes actually are, let's compute their relative optimality and visualize them.

```{r, fig.height = h, fig.width = w * 1.5}
# express values in terms of optimality
evd_survey_schemes$optimality <-
  ((max(evd_survey_schemes$mean_improvement) -
    evd_survey_schemes$mean_improvement) /
   evd_survey_schemes$mean_improvement) * 100
...
We can see that the relative return on investment for each of these different survey schemes are fairly comparable. All of the survey schemes are less than 5% from optimality. This result means that none of these schemes are especially bad for this particular simulation. However, in general, we would be best off identifying the optimal survey scheme by directly maximizing return on investment.
