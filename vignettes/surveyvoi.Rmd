...
ggplot(site_data) +
  geom_sf() +
  ggtitle("Sites") +
  labs(xlab="X coordinate", ylab="Y coordinate")
```
...
site_data %>%
  ...
  geom_sf(aes(color = value)) +
    ...
    facet_wrap(~ name) +
    labs(title = "Detection/non-detection data", xlab="X coordinate", ylab="Y coordinate")
```
...
site_data %>%
  ...
  geom_sf(aes(color = value)) +
    ...
    facet_wrap(~ name) +
    labs(title = "Number of previous surveys", xlab="X coordinate", ylab="Y coordinate")
```
...
p1 <- ggplot(site_data) +
      ...
      scale_color_viridis() +
      labs(title = "Survey cost", xlab="X coordinate", ylab="Y coordinate") +
      theme(legend.title = element_blank())
p2 <- ggplot(site_data) +
      geom_sf(aes(color = management_cost)) +
      scale_color_viridis() +
      labs(title = "Management cost", xlab="X coordinate", ylab="X coordinate") +
      theme(legend.title = element_blank())
...
site_data %>%
  ...
  geom_sf(aes(color = value)) +
    facet_wrap(~ var) +
    scale_color_viridis() +
    labs(title = "Environmental conditions", xlab="X coordinate", ylab="Y coordinate")
```

After simulating data for the sites, we will simulate data for the conservation feature. We set `proportion_of_survey_features` to 1 to ensure that this feature will be examined in future surveys.
...
site_data %>%
  ...
  facet_wrap(~name) +
  scale_color_viridis() +
  labs(title = "Modelled probabilities", xlab="X coordinate", ylab="Y coordinate")
```
...
We can see that the optimized survey scheme has the highest _return on investment_ of all the candidate survey schemes. To better understand how sub-optimal the candidate survey schemes are, let's compute their relative performance and visualize them.
