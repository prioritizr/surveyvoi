...
test_that("single species (without modelled probs in posterior)", {
  # data
  ## set seeds
  set.seed(123)
  ...
  site_data$n1 <- pmin(1, site_data$n1)
  site_data$p1 <- pmin(1 - 1e-5, site_data$p1)
...
test_that("single species (with modelled probs in posterior)", {
  # data
  ## set seeds
  set.seed(123)
  RandomFields::RFoptions(seed = 123)
  ...
  r1 <- approx_evdsi(
    ...
    n_approx_outcomes_per_replicate = 3)
  r2 <- r_approx_expected_value_of_decision_given_survey_scheme(
    dij = dij, nij = nij, pij = pij,
    survey_features = feature_data$survey,
    survey_sensitivity = feature_data$survey_sensitivity,
...
test_that("multiple species (without modelled probs in posterior", {
  # data
  ## set seeds
  set.seed(123)
  RandomFields::RFoptions(seed = 123)
  ...
  feature_data$survey <- TRUE
  feature_data$survey[2:3] <- FALSE
  total_budget <- sum(site_data$management_cost * 0.8)
  site_data$survey <- FALSE
  site_data$survey[which(site_data$f1 < 0.5)[1:2]] <- TRUE
  for (i in paste0("n", seq_len(n_f)))
    site_data[[i]] <- pmin(site_data[[i]], 1)
  ## create matrices for data
  site_data2 <- sf::st_drop_geometry(site_data)
  dij <- t(as.matrix(site_data2[, paste0("f", seq_len(n_f))], ncol = n_f))
  nij <- t(as.matrix(site_data2[, paste0("n", seq_len(n_f))], ncol = n_f))
...
test_that("multiple species (with modelled probs in posterior", {
  # data
  ## set seeds
  set.seed(123)
  RandomFields::RFoptions(seed = 123)
