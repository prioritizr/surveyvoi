#' Number of states
#'
#' Calculate the total number of occupancy and absence states in given the
#' size of a (\eqn{r_{ij}}) feature by planning unit matrix.
#'
#' @param x \code{numeric} number of cells in the matrix.
#'
#' @return \code{numeric} number of occupancy and absence states.
#'
#' @examples
#' # create rij data for 3 planning units and 2 features
...
n_states <- function(x) {
  ...
  assertthat::assert_that(assertthat::is.count(x), assertthat::noNA(x))
  # return result
  rcpp_n_states_wrapper(x)
}

#' N'th state
#'
#' Obtain a matrix with certain indices updated according to the n'th state.
#'
#' @param n \code{numeric} indicating the n'th state.
#'
#' @param matrix \code{matrix} indicating the set of conservation features
#'   and planning units. Columns denote planning units and rows
#'   denote features.
#'
#' @param idx \code{integer} vector containing matrix indices that should
#'   be updated.
#'
#' @return \code{matrix} indicating which features occupy which planning units
#' with zeros and ones.
#'
#' @examples
#' nth_state(2, matrix(nrow = 2, ncol = 2))
#'
#' @noRd
nth_state <- function(n, matrix, idx) {
  # assert arguments are valid
  assertthat::assert_that(
    assertthat::is.number(n), assertthat::noNA(n), all(n >= 0),
     is.matrix(matrix), nrow(matrix) > 0, ncol(matrix) > 0,
     is.numeric(idx), length(idx) > 0, anyDuplicated(idx) == 0,
     assertthat::noNA(idx), max(idx) <= length(matrix))
  # return result
  rcpp_nth_state_wrapper(n, matrix, idx - 1)
}

#' Which state?
#'
#' Identify the integer identifier associated with a matrix state.
#'
#' @param matrix \code{matrix} indicating the set of conservation features
#'   and planning units. Columns denote planning units and rows
#'   denote features.
...
#'
#' @noRd
which_state <- function(matrix, idx) {
  # assert arguments are valid
  assertthat::assert_that(
     is.matrix(matrix), nrow(matrix) > 0, ncol(matrix) > 0,
     is.numeric(idx), length(idx) > 0, anyDuplicated(idx) == 0,
     assertthat::noNA(idx), max(idx) <= length(matrix))
  # return result
  rcpp_which_state_wrapper(matrix, idx - 1)
}
