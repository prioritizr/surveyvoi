...
Here we provide a short example showing how to use the _surveyvoi R_ package to prioritize funds for ecological surveys. In this example, we will generate plans for conducting ecological surveys (termed "survey schemes") using simulated data for `r nrow(sim_sites)` sites and `r nrow(sim_features)` conservation features (e.g. bird species). To start off, we will set the seed for the random number generator for reproducibility load some R packages.
