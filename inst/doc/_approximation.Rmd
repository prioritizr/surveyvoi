...
Here we will examine the approximation method for conducting the value of information analyses. To achieve this, we will compare estimates from the approximation method against correct values derived from the exact method. A crucial part of the approximation method involves generating a subset of outcomes to _approximate_ the full set of outcomes, and so we will investigate several different methods for generating outcomes. We will also examine how the number of outcomes used in the approximation calculations affects the accuracy of the estimates.
...
site_data <- simulate_site_data(
  n_sites = 15, n_features = 1, max_number_surveys_per_site = 1,
  proportion_of_sites_missing_data = 5 / 15)
...
site_data$survey_scheme <-
  replace(rep(FALSE, nrow(site_data)), sample(which(site_data$n1 < 0.5), 4),
          TRUE)
