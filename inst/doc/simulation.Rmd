...
Now that we have identified suitable parameters for fitting the occupancy model, we will use these parameters to predict the probability that the feature occurs in each site.
...
Here, we will use the following equation to parametrise the ($B$) conservation benefit associated with managing a set of sites for conservation. Crucially, this benefit equation corresponds to sites prioritised for conservation management and not sites prioritised for surveys---meaning that we derive zero benefit if we only survey sites and don't manage any sites. To interpret this equation, let $J$ denote the set of sites (indexed by $j$), and $I$ denote the set of features (indexed by $i$). Also let $X_j$ contain binary (i.e. zero or one) values indicating if each site $i \in I$ is prioritized for conservation management or not, $Z_{ij}$ contain binary values indicating the presence or absence of features $i \in I$ occurring in sites $j \in J$, $\alpha_i$ denote the `alpha` column value(s), and $\gamma_i$ denote the `gamma` column value(s). This equation represents the objective function of the spatial prioritization component of the value of information calculations. It is conceptually similar to the objective function used in the Zonation decision support tool [@r1].

$$
B = \sum_{i \in I} \alpha_i \times \big( \sum_{j \in J} Z_{ij} X_j \big) ^{\gamma_i}
$$
...
ggplot(eqn_data, aes(x = x, y = y)) +
  ...
  xlab("Number of sites containing the feature held in prioritization") +
  ylab("Conservation benefit") +
  labs()
```

We can see that conservation benefit increases with the amount of the conservation feature held in the prioritization (i.e. the line is always increasing). We can also see that the line has a bend in it, and so the rate of conservation benefit accrued decreases as more of the feature is conserved. This is useful if we have multiple features, because it tells the conservation prioritization algorithm that once we have conserved "enough" of one feature that it can then focus on other features (broadly speaking).
