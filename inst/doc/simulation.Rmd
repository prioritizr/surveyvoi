...
site_data <- simulate_site_data(
  ...
  n_env_vars = 3, survey_cost_intensity = 5, management_cost_intensity = 100,
...
In this particular simulation, we can see that all of the survey schemes have a low return on investment (i.e. most values are close to zero). This means that none of these survey schemes would likely lead to a substantially better conservation outcome when considering the funds spent conducting them. Although surveying sites with the highest probability of occupancy is the best strategy -- in this particular situation -- because it has the best return on investment, can we improve on this? What is the optimal survey scheme?
...
assertthat::assert_that(sum(site_data$opt_scheme) > sum(site_data$geo_scheme))
assertthat::assert_that(!all(site_data$opt_scheme == (site_data$n1 > 0.5)))
```

We can see that the optimal survey scheme (`opt_scheme`) is different to the previous survey schemes. In particular, the optimal scheme selects more sites than the other survey schemes.
