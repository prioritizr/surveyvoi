...
ev_prime_current <-
  expand.grid(n = n_approx_states, replicate = seq_len(5)) %>%
  mutate(seed = sample.int(1e+8, nrow(.))) %>%
  plyr::ddply(c("n", "replicate"), function(x) {
    data.frame(value =
      expected_value_of_decision_given_current_information(
        site_data = site_data,
        feature_data = feature_data,
        site_occupancy_columns = "f1",
        site_probability_columns = "p1",
...
Now let's create a plot to visualize the results. In this plot, points correspond to estimates calculated used the approximate method. Also the line shows the correct value calculated using the exact method. We can see that, on average, the points get closer to the line at greater values on the x-axis. This confirms that the approximation method is more accurate when we use a greater proportion of number of states for the calculations. Indeed, the approximation method is roughly equal to the exact method when using a large number of states for the calculations.

```{r, fig.height = h, fig.width = w * 1.5}
# plot results
ggplot(ev_prime_current) +
geom_jitter(aes(x = n, y = value), height = 0) +
