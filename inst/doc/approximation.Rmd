...
ev_prime_current <-
  expand.grid(n = n_approx_states, replicate = seq_len(50)) %>%
...
ev_prime_certainty <-
  ...
  plyr::ddply(c("n", "replicate"), function(x) {
    data.frame(value =
      expected_value_of_decision_given_perfect_information(
        site_data = site_data,
        feature_data = feature_data,
        site_occupancy_columns = "f1",
        site_probability_columns = "p1",
...
# create plot
ggplot() +
geom_boxplot(aes(x = n, y = value, group = n2), point_data,
             width = abs(diff(range(n_approx_states))) * 0.05) +
...
We can see that, on average, the points get closer to the line at greater values on the x-axis. This demonstrates that the approximation method is more accurate when we use a greater number of states for the calculations. Indeed, the approximation method yields roughly equivalent values to the exact method when using a large number of states for the calculations.
